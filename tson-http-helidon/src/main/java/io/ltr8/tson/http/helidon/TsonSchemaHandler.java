...
/**
 * Serves a {@link TsonSchemaCatalog} through Helidon -- the routing around the catalog, and nothing else. The
 * lookup, the identity-path rule and the cache policy live in the catalog, shared with every other adapter.
 *
 * <p>Register it on a catch-all route, since the paths it serves are the identity paths of the schemas it holds
 * rather than anything Helidon's router knows about:
 *
 * <pre>{@code
 * routing.any(TsonHandler.asHandler(codec, TsonSchemaHandler.of(orderSchema, problemSchema)));
 * }</pre>
 */
