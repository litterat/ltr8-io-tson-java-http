 ...
 * }</pre>
 *
 * <p>The boundary does the same four things as the JDK and Javalin adapters', and for the same reasons: it negotiates {@code
 ...
 * violations the read found. The exception goes to a {@link System.Logger}.
 *
...
public interface TsonHandler {
    ...
    static void install(HttpRouting.Builder routing, TsonHttpCodec codec) {
        routing.error(TsonHttpException.class, (request, response, failure) ->
                Boundary.fail(codec, Boundary.context(request, response, codec), failure));
        routing.error(UnsupportedTypeException.class, (request, response, failure) -> {
            TsonContext tson = Boundary.context(request, response, codec);
    ...
    final class Boundary {
         ...
         */
        static TsonContext context(ServerRequest request, ServerResponse response, TsonHttpCodec codec) {
            TsonContext tson = new TsonContext(request, response, codec);
            try {
                tson.representation(codec.negotiate(tson.header("Accept")));
